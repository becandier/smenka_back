...
| PATCH | `/api/v1/organizations/{id}/members/{user_id}` | Задать/сбросить `display_name` участника в org (`member_display_name`) | Bearer (owner/admin/super_admin) |
...
| `services/payroll.py` | История ставок участников (CRUD, `PayrollError`), действующие ставки batch-запросом (DISTINCT ON), расчёт payroll/my-earnings «на лету» (+штрафы: `include_penalties`, `net`), суточная разбивка (`_build_breakdown`, посуточное округление) + экспорт `.xlsx` (`export_org_payroll`/`_build_payroll_xlsx`); **member_display_name:** `display_name` в `items[]` обеих проекций (агрегат/`granularity` с разбивкой), тем же batch-запросом `OrganizationMember`, что и ставки; **work_schedules (R8):** `_calc_earnings` расширен `overtime_seconds`/`planned_seconds`/`planned_amount_minor`/`delta_amount_minor`/`late_count`/`late_seconds_total` — `paid_seconds = worked + approved-переработка` (по нему `gross` для `hourly`; `per_shift` игнорирует), план — окно графика или факт (без графика); округление плана — тем же правилом, что и факта (round-once на итог/день) |
