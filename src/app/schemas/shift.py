...
class ShiftStartRequest(BaseModel):
    ...
    work_location_id: str | None = Field(
        default=None,
        description="UUID точки смены. При включённой геопроверке — необязательный явный "
        "выбор: точка валидируется (принадлежит организации и координаты внутри её "
        "радиуса, иначе 404 WORK_LOCATION_NOT_FOUND / 403 WORK_LOCATION_OUT_OF_RANGE) и "
        "используется как есть; не передан — сервер определяет ближайшую из совпавших зон "
        "(как раньше). При выключенной гео обязателен, если у организации включён "
