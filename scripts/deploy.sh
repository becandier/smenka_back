...
# Каждый pull :latest оставляет предыдущий образ без тега (dangling). Без чистки
# они копятся на диске VPS: к 2026-09-13 набралось 125 штук / 4 ГБ. Удаляем только
# образы без тега и без контейнеров; откат — через ghcr, а не через локальный кэш.
echo "▶ удаление устаревших образов"
docker image prune -f || echo "  ⚠ prune не выполнен — на деплой не влияет"
